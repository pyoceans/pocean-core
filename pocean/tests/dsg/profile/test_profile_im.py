...
class TestIMPStrings(unittest.TestCase):
    def setUp(self):
        self.df = pd.read_csv(
            os.path.join(os.path.dirname(__file__), "resources", "basis_2011.csv"),
            parse_dates=["time"],
        )
    ...
    def test_write_nc(self):
        ...
        axes = {"t": "time", "x": "longitude", "y": "latitude", "z": "z", "profile": "stationid"}

        with IncompleteMultidimensionalProfile.from_dataframe(
            self.df, single_tmp, axes=axes, mode="a"
        ) as ncd:
