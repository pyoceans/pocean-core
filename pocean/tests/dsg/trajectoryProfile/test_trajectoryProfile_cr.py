...
class TestContinousRaggedTrajectoryProfile(unittest.TestCase):
    def setUp(self):
        ...
        self.missing_time = os.path.join(
            os.path.dirname(__file__), "resources", "cr-missing-time.nc"
        )
        self.nan_locations = os.path.join(
            os.path.dirname(__file__), "resources", "cr-nan-locations.nc"
        )
    ...
    def test_crtp_dataframe_single_unique_dims(self):
        ...
        with ContiguousRaggedTrajectoryProfile(self.single) as ncd:
            df = ncd.to_dataframe(axes=axes)
            with ContiguousRaggedTrajectoryProfile.from_dataframe(
                df, tmpnc, axes=axes, unique_dims=True
            ) as result_ncd:
